...
void sortSandalByNameDesc(Sandal daftarSandal[], int jumlahSandal);
void sortSandalByPriceAsc(Sandal daftarSandal[], int jumlahSandal);
void sortSandalByStockDesc(Sandal daftarSandal[], int jumlahSandal);
void menuUrutkan(Sandal daftarSandal[], int jumlahSandal);

void sortSandalByNameDesc(Sandal daftarSandal[], int jumlahSandal) {
    for (int i = 0; i < jumlahSandal - 1; i++) {
        for (int j = 0; j < jumlahSandal - i - 1; j++) {
            if (daftarSandal[j].nama < daftarSandal[j + 1].nama) {
                Sandal temp = daftarSandal[j];
                daftarSandal[j] = daftarSandal[j + 1];
                daftarSandal[j + 1] = temp;
            }
        }
    }
}
...
void sortSandalByPriceAsc(Sandal daftarSandal[], int jumlahSandal) {
    for (int i = 1; i < jumlahSandal; i++) {
        Sandal key = daftarSandal[i];
        int j = i - 1;

        while (j >= 0 && daftarSandal[j].harga > key.harga) {
            daftarSandal[j + 1] = daftarSandal[j];
            j--;
        }

        daftarSandal[j + 1] = key;
    }
}
...
void sortSandalByStockDesc(Sandal daftarSandal[], int jumlahSandal) {
    for (int i = 0; i < jumlahSandal - 1; i++) {
        int max_idx = i;
        for (int j = i + 1; j < jumlahSandal; j++) {
            if (daftarSandal[j].stok > daftarSandal[max_idx].stok) {
                max_idx = j;
            }
        }
        if (max_idx != i) {
            Sandal temp = daftarSandal[i];
            daftarSandal[i] = daftarSandal[max_idx];
            daftarSandal[max_idx] = temp;
        }
    }
}
...
void menuUrutkan(Sandal daftarSandal[], int jumlahSandal) {
    int pilihanUrut;
    cout << "\n----- MENU PENGURUTAN SANDAL -----" << endl;
    cout << "1. Urutkan berdasarkan nama (Z-A)" << endl;
    cout << "2. Urutkan berdasarkan harga (Rendah-Tinggi)" << endl;
    cout << "3. Urutkan berdasarkan stok (Tinggi-Rendah)" << endl;
    cout << "4. Kembali" << endl;
    cout << "Pilihan: ";
    cin >> pilihanUrut;
    ...
    switch (pilihanUrut) {
        case 1:
            sortSandalByNameDesc(daftarSandal, jumlahSandal);
            cout << "Sandal telah diurutkan berdasarkan nama secara menurun (Z-A)!" << endl;
            tampilkanDaftarSandal(daftarSandal, jumlahSandal);
            break;
        case 2:
            sortSandalByPriceAsc(daftarSandal, jumlahSandal);
            cout << "Sandal telah diurutkan berdasarkan harga dari terendah ke tertinggi!" << endl;
            tampilkanDaftarSandal(daftarSandal, jumlahSandal);
            break;
        case 3:
            sortSandalByStockDesc(daftarSandal, jumlahSandal);
            cout << "Sandal telah diurutkan berdasarkan stok dari terbanyak!" << endl;
            tampilkanDaftarSandal(daftarSandal, jumlahSandal);
            break;
        case 4:
    ...
    do {
        ...
        switch (menuLogin) {
            ...
            case 4:
                ...
                break;
            case 5:                                  
                menuUrutkan(daftarSandal, jumlahSandal);
                break;
            case 6:                            
                sudahLogin = false;
